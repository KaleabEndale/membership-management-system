...
exports.back = async(req,res)=>{
    ...
    if(!token){
        ...
        const decode =  jwt.verify(token,process.env.ACCESS_TOKEN,async(err,aos)=>{
            if(err){
                 res.render('mhome')
            }else{ 
...
exports.viewmember = async(req,res)=>{
    ...
    if(!member){
        res.render('viewmember',{username:username,er:'member doesnot exist'})
    }else{
      ...
      if(!m){
       ...
       if(!sub){
        res.render('viewmember',{mi:m.id,mn:m.username,mc:m.contact,me:m.email,ma:m.address})
       }else{
  if(sub.how == 'session' || sub.how == 'subscription'){
        if(sub.type == 'lifetime'){
            ...
            const paymentTimestamp = new Date( paymentIntent.created * 1000)
             ...
             res.render('viewmember',{mi:m.id,mn:m.username,mc:m.contact,me:m.email,ma:m.address,mmt:name,mms:'active',med:'never',mod:0,mmc:currency})
...
res.render('viewmember',{mi:m.id,mn:m.username,mc:m.contact,me:m.email,ma:m.address,mmt:name,mms:status,med:expiredate,mod:outstandingdue,mmc:currency})
...
exports.findmember = async(req,res)=>{
    ...
    const member = await members.findOne({where:{username:username}})
    if(!member){
        res.render('findmember',{username:username,er:'member doesnot exist'})
    }else{
        res.render('editmember',{username:username,email:member.email,contact:member.contact,address:member.address})
    }
}
...
exports.editmember = async(req,res)=>{
 ...
 res.render('editmember',{username:username,email:email,contact:contact,address:address,mu:'member updated successfully'})
}
...
exports.cm = async(req,res)=>{
    ...
    const m = await members.findOne({where:{username:username}})
    if(!m){
        res.render('pfm',{username:username,showButton:false,er:'user doesnot exist'})
    }else{
        const md = await mds.findOne({where:{id:m.id}})
    if(!md){
        res.render('pfm',{username:username,email:m.email,showButton:true})
    }else{
        res.render('pfm',{username:username,showButton:false,er:'user already paid'})
    }
}
  } catch (error) {
        console.error('Error fetching data from stripe , ERROR : ', error.message);
...
if(currency == 'etb'){
    ...
    price = (price * 1.12).toFixed(3)
     ...
     if(type=='lifetime'){
        ...
        cashSubscription(req,res,id,username,email,type,price,currency,recurrence)
     }  
    } catch (error) {
        console.error('Error fetching data from stripe , ERROR : ', error.message);
        res.status(500).send('Error fetching data from stripe');
...
exports.viewstaff = async(req,res)=>{
    ...
    if(!staff){
        ...
        res.render('viewstaff',{username:username,id:staff.id,username1:staff.username,email:staff.email,contact:staff.contact,address:staff.address})
    }
}
...
exports.findstaff = async(req,res)=>{
    ...
    if(!staff){
        ...
        res.render('editstaff',{username:username,email:staff.email,contact:staff.contact,address:staff.address})
    }
}
...
exports.editstaff = async(req,res)=>{
 ...
 await staffs.update({contact:contact,email:email,address:address},{where:{username:username}})
 res.render('editstaff',{username:username,email:email,contact:contact,address:address,mu:'staff updated successfully'})
}
