   ...
   if(!errors.isEmpty()){ 
    ...
    if(!admin){
        ...
        const sent = await transporter.sendMail({
         ...
         const token =  jwt.sign({email:email,passSent:passSent},process.env.RESET_ACCESS_TOKEN)
         res.cookie('token',token,{
             httpOnly:true,
             secure:true,
             maxAge:1800000
...
exports.adminrecoverpassword = async(req,res)=>{
    ...
    if(token){
        const decode =  jwt.verify(token,process.env.RESET_ACCESS_TOKEN,async(err,t)=>{
            if(err){
                res.render('afp',{email:t.email,ade:'account doesnot exist '})
            }else{ 
               if(sent == t.passSent){
                const hashedpass = await bcrypt.hash(newpassword,10)
                  admins.update({password:hashedpass},{where:{email:t.email}})
                  res.render('admin')
               }else{
                res.render('apr',{newpassword:newpassword,sent:sent,ade:'wrong pass'})
               }
...
exports.staffrecoverpassword = async(req,res)=>{
    ...
    if(token){
        const decode =  jwt.verify(token,process.env.RESET_ACCESS_TOKEN,async(err,t)=>{
            if(err){
                res.render('sfp',{email:t.email,ade:'account doesnot exist '})
            }else{ 
               if(sent == t.passSent){
                const hashedpass = await bcrypt.hash(newpassword,10)
                  staffs.update({password:hashedpass},{where:{email:t.email}})
                  res.render('staff')
               }else{
               res.render('spr',{newpassword:newpassword,sent:sent,ade:'wrong pass'})
               }
...
if(!member){
    res.render('mlogin',{username:username,password:password,e:"wrong password or username"})   
}else{
   ...
   if(p){
    ...
    if(!sub){
       ...
       return res.render('mprofile',{mn:member.username,me:member.email,mc:member.contact,ma:member.address})    
    }else{
      if(sub.how == 'session' || sub.how == 'subscription'){
        if(sub.type == 'lifetime'){
            ...
            const paymentTimestamp = new Date( paymentIntent.created * 1000)
             ...
             logs(username,'login','user logged in successfully',ip)
             res.render('mprofile2',{showButton:false,si:sub.subid,mn:member.username,mc:member.contact,me:member.email,ma:member.address,mmt:name,mms:'active',med:'never',mod:0,mmc:currency})
        }else{
...
outstandingdue = outstandingdue / 100
  ...
  if(expdate < currentDate){
    logs(username,'login','user logged in successfully',ip)
    res.render('mprofile2',{showButton:true,msg:'subscription expired',si:sub.subid,mn:member.username,mc:member.contact,me:member.email,ma:member.address,mmt:name,mms:status,med:expiredate,mod:outstandingdue,mmc:currency})
}
else{
    logs(username,'login','user logged in successfully',ip)
    res.render('mprofile2',{showButton:true,si:sub.subid,mn:member.username,mc:member.contact,me:member.email,ma:member.address,mmt:name,mms:status,med:expiredate,mod:outstandingdue,mmc:currency})    
}
   }
    }
...
exports.memberrecoverpassword = async(req,res)=>{
    ...
    if(token){
        const decode =  jwt.verify(token,process.env.RESET_ACCESS_TOKEN,async(err,t)=>{
            if(err){
                res.render('mfp',{email:t.email,ade:'account doesnot exist '})
            }else{ 
               if(sent == t.passSent){
                const hashedpass = await bcrypt.hash(newpassword,10)
                 await members.update({password:hashedpass},{where:{email:t.email}})
               const m = await members.findOne({where:{email:t.email}}) 
                  logs(m.username,'recover','user recovered password successfully',ip)
                  res.render('mlogin')
               }else{
                res.render('mpr',{newpassword:newpassword,sent:sent,ade:'wrong pass'})
               }
