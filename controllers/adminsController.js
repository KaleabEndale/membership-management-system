    ...
    try {
        ...
        for (let i = 0; i < sessions.data.length; i++) {
            ...
            if (session.client_reference_id === 'membership') {
                    ...
                    const sy = new Date(sub.current_period_end * 1000).getFullYear();
                    const sm = new Date(sub.current_period_end * 1000).getMonth() + 1;
                    if(sy==input){
                    renew[sm - 1].push({ status: sub.status });
                    }
                }else{
                     const sy = new Date(sub.canceled_at * 1000).getFullYear();
                    const sm = new Date(sub.canceled_at * 1000).getMonth() + 1;
                    if(sy==input){
                    months[sm - 1].push({ status: sub.status });
                    }
                }
        ...
        for (let i = 0; i < subscriptions.data.length; i++) {
            const subscription = subscriptions.data[i];
            if (subscription.metadata.client_reference_id === 'membership') {
                  if (subscription.status === 'active') {
                    const sy = new Date(subscription.current_period_end * 1000).getFullYear();
                    const sm = new Date(subscription.current_period_end * 1000).getMonth() + 1;
                    if(sy==input){
                    renew[sm - 1].push({ status: subscription.status });
                    }
                }else{
                     const sy = new Date(subscription.canceled_at * 1000).getFullYear();
                    const sm = new Date(subscription.canceled_at * 1000).getMonth() + 1;
                    if(sy==input){
                    months[sm - 1].push({ status: subscription.status });
                    }
                }
